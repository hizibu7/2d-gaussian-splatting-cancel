...
def step(it, g, photo_loss, viewpoint_cam, mp, K=1000):
    if not _S["init"]:
        ...
        _S["out"]=os.path.join(mp,"cancellation_v3.npz"); _S["init"]=True
    t0=time.time()
    P=g._xyz.shape[0]
    ...
    if it%2000==0:
        print(f"[v3@{it}] {el*1000:.0f}ms tot={_S['elapsed']:.0f}s avg_cancel_t={cancel_t.mean().item():.3f} corr~{((max_sc*cancel_t).sum()/(max_sc.sum()*cancel_t.mean()+1e-8)).item():.2f}",flush=True)
...
def finalize():
    ...
    print(f"[v3] saved {_S['out']} calls={_S['calls']} total={_S['elapsed']:.1f}s",flush=True)
