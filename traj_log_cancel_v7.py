...
def log_at_densify(iteration, gaussians, max_grad, min_opacity, extent, mp):
    if not _S["init"]:
        ...
        _S["out"]=os.path.join(mp,"densify_log_v7.npz"); _S["init"]=True
    ...
    rec = {"iter":iteration,"n_gauss":len(grads_safe),"grad":grads_safe,"max_scale":max_sc,
           ...
           "wc":wc.astype(np.uint8),"wp":wp.astype(np.uint8),"max_grad":float(max_grad)}
    _S["data"].append(rec); _S["calls"] += 1
    if _S["calls"] % 10 == 1:
        n=len(grads_safe);ps=pt.sum();sp=ws.sum();cl=wc.sum()
        print(f"[v7@{iteration}] N={n} pass={ps}",flush=True)
...
def finalize():
    ...
    for k in ["grad","max_scale","opacity","pt","ws","wc","wp"]:
